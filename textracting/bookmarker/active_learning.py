...
def display_page(docid, page, line=None, mode=settings.dataset_version):
...
def display_df(docid, table):
    dfs = borehole_tables.get_tables(docid)
    df = dfs[table-1]
...
def al_input_loop(learner, inst, docid, n_queries, classes, page=None, line=None, mode=settings.dataset_version):
...
def al_data_prep(data, y_column, limit_cols=None, mode=settings.dataset_version):  # to generalise further, should take limit_cols param and generalise data_prep
    default_drop = ['DocID', 'TagMethod']
    if not limit_cols:
