...
def display_line(docid, page, line):
    pg_path = settings.get_report_page_path(int(docid), int(page))  # docid, page
    image = Image.open(pg_path)
    display.display(image)
...
def active_learning(data, n_queries, y_column, estimator=sklearn.ensemble.RandomForestClassifier()):
    line = False
    ...
    X_initial, Y_initial, X_pool, y_pool, ref_docids, ref_idx = al_data_prep(data, y_column)
    ...
    for i in range(n_queries):
        idx = query_idx[i]
        y = al_loop(learner, query_inst[i], ref_docids.iloc[idx], n_queries, classes, line=line)
        y_new[i] = y
...
def passive_learning(data, y_column, estimator=sklearn.ensemble.RandomForestClassifier()):
    print("training with all labelled samples")
    data = data.dropna(subset=[y_column])
    X, Y = data_prep(data, limit_cols=['DocID', 'TagMethod'], y_column=y_column)
...
def train(data, y_column, n_queries, estimator, datafile):
    ...
    if n_queries > 0:
        updated_data, accuracy, learner = active_learning(data, n_queries, y_column, estimator)
        updated_data.to_csv(datafile, index=False)  # save slightly more annotated dataset
    else:
        accuracy, learner = passive_learning(data, y_column, estimator)
    return accuracy, learner


def al_loop(learner, inst, docid, n_queries, classes, line=False):
    print("Waiting to display next....")
    display.clear_output(wait=True)
    pred = learner.predict(inst.reshape(1, -1))
    #preds.append(pred[0])
    if line:
        display_line(int(docid), int(inst[0]), int(inst[1]))
    else:
        display_page(int(docid), int(inst[0]))  # docid, pagenum
