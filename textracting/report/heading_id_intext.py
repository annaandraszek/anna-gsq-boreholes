## @file
# Heading identification in-text module: Functions for finding headings in the report text
# by Anna Andraszek
...
import textdistance
import numpy as np
from report import active_learning, machine_learning_helper as mlh, heading_id_toc
...
def create_dataset(datafile = paths.get_dataset_path(name), docid=False): #datafile=settings.dataset_path + 'heading_id_intext_dataset.csv', docid=False):
...
def get_headings_intext(data, toc_page=True, mode=paths.dataset_version):
    if not toc_page:
...
if __name__ == '__main__':
    data_path = paths.dataset_path + 'heading_id_intext_dataset.csv'
    #data = pd.read_csv(data_path)
    #create_dataset()
    #train()
    #edit_dataset(data_path)
    data = pd.read_csv(data_path)
    get_headings_intext(data, True, mode=paths.production)
