## @file
# TOC page classification module: Functions for classifying a page as a Table of Contents
# by Anna Andraszek
...
def create_dataset():
    ...
    prev_toc_dataset = paths.dataset_path + 'toc_dataset.csv'
...
def get_toc_pages(df, mode=paths.dataset_version):
    prevpgtoc = False
    res = []
    for i, row in df.iterrows():
        if prevpgtoc and mode != paths.production:
            row['PrevPageTOC'] = 1
...
if __name__ == "__main__":
    ...
    train(n_queries=0, mode=paths.production)
    #automatically_tag()
    # check_tags()
    # train(all=True)
    # tag_prevpagetoc()
