## @file
# Heading identification in TOC module: Functions for finding headings in the table of contents
# by Anna Andraszek

from report.toc_classification import get_toc_pages
import glob
import json
...
from report import active_learning
...
limit_cols = ['SectionPrefix', 'SectionText', 'SectionPage', 'TagMethod', 'DocID', 'PageNum', 'LineNum']
include_cols = ['Left', 'Top', 'ProcessedText']
seq_len=20


class PreprocessText(TransformerMixin, BaseEstimator):
    def fit(self, x, y=None):
        return self
    def transform(self, data):
        trans_data = pre_process_id_dataset(data)
        return trans_data
...
class NeuralNetwork(): #give this arguments like: model type, train/test file
    ...
    def __init__(self, mode=paths.dataset_version):
    ...
    def predict(self, strings, encode=False, mode=paths.dataset_version):
        if encode:
...
def train(n_queries=10, mode=paths.dataset_version):
    if not os.path.exists(paths.get_dataset_path(name, mode)):
        #if not os.path.exists(settings.get_dataset_path(name, mode)):
        create_dataset(datafile=paths.get_dataset_path(name, mode))
        df = pre_process_id_dataset(datafile=paths.get_dataset_path(name, mode))
        df.to_csv(paths.get_dataset_path(name, mode))
...
def pre_process_id_dataset(datafile=paths.get_dataset_path('heading_id_toc'), training=True):
    ...
    if 'Text' in df.columns.values:
        text_ln = 'Text'
    elif 'LineText' in df.columns.values:
        text_ln = 'LineText'
    df['SectionPrefix'], df['SectionText'] = zip(*df[text_ln].map(split_prefix))
...
def create_dataset(datafile=paths.get_dataset_path(name)):
    df = pd.DataFrame(columns=columns)
    lines_docs = sorted(glob.glob('training/restructpageinfo/*.json'))
    toc_df = pd.read_csv(paths.get_dataset_path('toc'))
...
def get_toc_headings(df, mode=paths.dataset_version, masked=False):
