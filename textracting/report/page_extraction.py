## @file
# Page number extraction module: Functions for extracting page numbers from marginals known to contain a page number
# by Anna Andraszek

from __future__ import print_function
import pandas as pd
import paths
from report.page_identification import transform_text_wrapper
from report import page_identification, active_learning, machine_learning_helper as mlh
import os
...
def train(n_queries=10, mode=paths.dataset_version):
    if not os.path.exists(paths.get_dataset_path(name, mode)):
        ...
        df.to_csv(paths.get_dataset_path(name, mode), index=False)
    nn = NeuralNetwork()
...
def get_page_nums(marginals, ml_only=False, mode=paths.dataset_version): # given a dataset of marginals of the document
...
if __name__ == '__main__':
    ...
    #nn.train()
    run_model(model_name='mask_nn', model_type='NN', mode=paths.production)
