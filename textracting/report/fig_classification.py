## @file
# Figure classification module: Functions for classifying a page as a figure (map, image, non-textual entity)
# by Anna Andraszek
...
import os
import matplotlib
from report import machine_learning_helper as mlh, active_learning as al
...
def train(n_queries=10, mode=paths.dataset_version):  # datafile=settings.get_dataset_path(name), model_file=settings.get_model_path(name),
...
def get_fig_pages(data, mode=paths.dataset_version): #docid, docinfo, doclines):  # change usages of this to pass fig dataset
